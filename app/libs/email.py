...
from app.libs.email_templates import reset_password_template, invite_user_template


def send_invite_user_mail(email="none", first_name="", lastname="", password="",):
    try:
        msg = Message(subject=f'Welcome to Prayer App!',
                      sender=MAIL_USERNAME, recipients=[email])
        msg.html = invite_user_template(first_name, lastname, password)
        mail.send(msg)
        return 0
